...
public class Chessboard extends JComponent {
    ...
    public ArrayList<List<String>> chessBoardList=new ArrayList<>();
    public ArrayList<ArrayList<Integer>> scoreLists=new ArrayList<>();
    public ArrayList<ArrayList<SquareComponent>> eatenLists=new ArrayList<>();
    ...
    private void initAdvisorOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new AdvisorChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
    ...
    private void initCannonOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new CannonChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
    ...
    private void initChariotOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new ChariotChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
    ...
    private void initGeneralOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new GeneralChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
    ...
    private void initHorseOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new horseChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
    ...
    private void initMinisterOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new MinisterChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
    ...
    private void initSoldierOnBoardReverse(int row, int col, ChessColor color) throws IOException {
        SquareComponent chesscomponent = new SoldierChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, CHESS_SIZE);
        chesscomponent.setVisible(true);
        chesscomponent.setReversal(true);
        putChessOnBoard(chesscomponent);
            ...
            if(chessData.get(i).length()==2){
                if(squareComponents[chessData.get(i).charAt(0)-48][chessData.get(i).charAt(1)-48].isReversal()){
                    ...
                    if(number==1){
                        ...
                        ChessGameFrame.getStatusLabel1().setText(String.format("玩家1(%s)",getCurrentColor().getName()));
                ...
                if(squareComponents[chessData.get(i).charAt(0)-48][chessData.get(i).charAt(1)-48].canMoveTo(squareComponents,squareComponents[chessData.get(i).charAt(2)-48][chessData.get(i).charAt(3)-48].getChessboardPoint(),squareComponents[chessData.get(i).charAt(0)-48][chessData.get(i).charAt(1)-48],squareComponents[chessData.get(i).charAt(0)-48][chessData.get(i).charAt(1)-48].getChessboardPoint())){
                  eatenHistory.add(squareComponents[chessData.get(i).charAt(2)-48][chessData.get(i).charAt(3)-48]);
    ...
    public List<String> getChessStringListUndo(){
        List<String> chessStringList=new ArrayList<>();
        for(int i=0;i<8;i++){
            StringBuilder builder=new StringBuilder();
            for(int j=0;j<4;j++){
                if(squareComponents[i][j].getChessColor()== ChessColor.BLACK&&!squareComponents[i][j].isReversal()){
                    if(squareComponents[i][j] instanceof AdvisorChessComponent){
                        builder.append('S');
                    }else if(squareComponents[i][j] instanceof CannonChessComponent){
                        builder.append('P');
                ...
                }else if (squareComponents[i][j].getChessColor()== ChessColor.RED&&!squareComponents[i][j].isReversal()){
                    if(squareComponents[i][j] instanceof AdvisorChessComponent){
                        builder.append('s');
                    }else if(squareComponents[i][j] instanceof CannonChessComponent){
                        builder.append('p');
                ...
                }else if((squareComponents[i][j].getChessColor()== ChessColor.BLACK)&&(squareComponents[i][j].isReversal())){
                    if(squareComponents[i][j] instanceof AdvisorChessComponent){
                        builder.append('T');//S
                    }else if(squareComponents[i][j] instanceof CannonChessComponent){
                        builder.append('Q');//P
    ...
    public boolean undo() throws IOException {
        if(ClickController.clickNumber==0){
            JOptionPane.showMessageDialog(null, "第一步不能悔棋！","" ,JOptionPane.ERROR_MESSAGE);
            return false;
        }
        chessBoardList.remove(chessBoardList.size()-1);
        scoreLists.remove(scoreLists.size()-1);
        eatenLists.remove(eatenLists.size()-1);
        initiateEmptyChessboard();
        for(int i=0;i<8;i++){
            for(int j=0;j<4;j++){
                if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='S'){
                    initAdvisorOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='P'){
                    initCannonOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='C'){
                    initChariotOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='J'){
                    initGeneralOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='M'){
                    initHorseOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='X'){
                    initMinisterOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='B'){
                    initSoldierOnBoard(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='s'){
                    initAdvisorOnBoard(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='p'){
                    initCannonOnBoard(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='c'){
                    ...
                    initSoldierOnBoard(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='T'){
                    initAdvisorOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='Q'){
                    initCannonOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='D'){
                    initChariotOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='K'){
                    initGeneralOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='N'){
                    initHorseOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='Y'){
                    initMinisterOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='A'){
                    initSoldierOnBoardReverse(i,j,ChessColor.BLACK);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='t'){
                    initAdvisorOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='q'){
                    initCannonOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='d'){
                    initChariotOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='k'){
                    initGeneralOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='n'){
                    initHorseOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='y'){
                    initMinisterOnBoardReverse(i,j,ChessColor.RED);
                }else if(chessBoardList.get(chessBoardList.size()-1).get(i).charAt(j)=='a'){
                    initSoldierOnBoardReverse(i,j,ChessColor.RED);
                }
                squareComponents[i][j].repaint();
            }
        }
        ...
        scoreList.set(0,scoreLists.get(scoreLists.size()-1).get(0));
        scoreList.set(1,scoreLists.get(scoreLists.size()-1).get(1));
        for(ArrayList<Integer> score:scoreLists){
            for(Integer e:score){
                System.out.println(e);
            }
        }

        System.out.println(scoreLists.size());
        clickController.swapPlayer();
        clickController.winGame();
        if(chessBoardList.size()==1){
            ChessGameFrame.getStatusLabel().setText("Reveal a chess to start!");
        }
        ClickController.clickNumber--;
        return true;
    }
