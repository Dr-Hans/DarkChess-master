...
public abstract class SquareComponent extends JComponent {
    ...
    protected static final Font CHESS_FONT = new Font("Rockwell", Font.BOLD, 36);//换图片需删
    ...
    protected int point;
    protected int level;
    protected boolean canEatCannon;
    ...
    public boolean canMoveTo(SquareComponent[][] chessboard, ChessboardPoint destination, SquareComponent lastPoint,ChessboardPoint start) {
        ...
        boolean distance;
        boolean compareLevel;
        boolean cannonEat;
        if(Math.abs(start.getX()- destination.getX())+Math.abs(start.getY()- destination.getY())>1 ){
            distance=false;
        }else{
            distance=true;
        }//距离
        if(lastPoint.level-destinationChess.level>10000||(lastPoint.level-destinationChess.level>(-10000)&&lastPoint.level-destinationChess.level<0)){
            compareLevel=false;
        }else{
            compareLevel=true;
        }
        if(lastPoint instanceof SoldierChessComponent&&destinationChess instanceof CannonChessComponent){
            cannonEat=false;
        }else{
            cannonEat=true;
        }

        return (destinationChess.isReversal&&(distance)&&(compareLevel)&&cannonEat)|| destinationChess instanceof EmptySlotComponent;
